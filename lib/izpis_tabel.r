...
cairo_pdf("slike/tabela_nutrition.pdf", height=8, width=8.5, family="Courier New")
grid.table(prikaz_nutrition, gp=gpar(fontsize=8))
dev.off()

prikaz_sestavine <- sestavine[, c(2:4, 7:9)]

cairo_pdf("slike/tabela_sestavine.pdf", height=5, width=8.5, family="Courier New")
grid.table(prikaz_sestavine, gp=gpar(fontsize=8))
dev.off()

prikaz_lokacije <- lokacije[1:40, ]

cairo_pdf("slike/tabela_lokacije.pdf", height=10, width=8.5, family="Courier New")
